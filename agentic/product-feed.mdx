...
// 1. Fetch user's intent profile (companion API — not yet in SDK)
const profile = await fetch(`${BASE}/companion/profile/${userId}`, {
  headers,
}).then(r => r.json())

// 2. Get personalized recommendations (companion API)
const recs = await fetch(`${BASE}/companion/recommendations/${userId}`, {
  headers,
}).then(r => r.json())
