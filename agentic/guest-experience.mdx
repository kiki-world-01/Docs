...
// 3. After user signs up, merge their session into the real account.
// POST /companion/guest/convert is not yet in the generated SDK
// (only POST /companion/guest/session is, as `companionGuestSession`).
// Call it over HTTP for now.
