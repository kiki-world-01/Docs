...
description: A complete reference implementation — building a personal shopping agent on Podium with Telegram, Claude, and Privy wallets
---
...
<Info>
  Source code: [github.com/kiki-world-01/beauty-companion](https://github.com/kiki-world-01/beauty-companion)
</Info>
...
The bot includes a local Claude-powered recommendation agent:
...
4. Claude returns a ranked list of product IDs sorted by relevance
5. Fallback: if Claude fails, returns products in catalog order

This demonstrates a hybrid approach — Podium provides the AI-ranked `recommendations` endpoint server-side, and the companion can optionally layer its own Claude-based ranking on top.
...
| AI | Anthropic Claude | Product recommendation ranking |
