...
## Email Insight Surface

Memory powers a personalized email insight system. The `generateEmailInsight` function composes a multi-sentence, first-person summary from structured memory — attributes, goals, concerns, avoidances, and negatively-reviewed products — capped at 500 characters for email readability.

```json
{
  "sageInsightSummary": "I know you're focused on hydration and anti-aging for your sensitive, dry skin. You love The Ordinary and CeraVe, but fragrance is a no-go. That retinol serum was too intense, so I'll stick to gentler actives for now. Your budget sweet spot for serums is $15–45."
}
```

This insight is:

- **Synced to the email automation system** as a contact property after every summary job
- **Used as a template variable** (`{{sageInsightSummary}}`) in lifecycle emails — particularly the post-onboarding email
- **Included in the `memory_queued` SSE event** as `insightSummary` so frontends can render a "here's what I'm holding" card

The first time a summary is generated (when no prior memory exists), the system detects this as a milestone and triggers the post-onboarding email with the rich insight text.

## User Preference Embeddings

Each user accumulates a **preference embedding** (`enrichmentVec`) stored on their intent profile. This vector is a mathematical representation of the user's taste, computed from their interaction history:

| Interaction | Weight | Effect |
|---|---|---|
| Purchase | 5x | Strong pull toward similar products |
| Rank Up / Save | 2x | Moderate positive signal |
| View / Click | 0.5x | Weak exploratory signal |
| Rank Down / Dismiss | -2x | Push away from similar products |

The preference vector is L2-normalized and updated automatically on high-signal interactions. During recommendation ranking, **cosine similarity** between the user's vector and each candidate product's embedding creates a personalization signal that's independent of explicit filters.

<Tip>
Preference embeddings start empty and build organically. For new users, ranking falls back to population-level signals (popularity, quality score, brand tier) until enough interactions create a meaningful vector.
</Tip>

See [Ranking Intelligence](/agentic/ranking-intelligence) for how embeddings integrate with the full ranking pipeline.
...
graph LR
    ...
    Memory --> Email["Lifecycle Emails"]
    Memory --> Embeddings["Preference Embeddings"]
```
...
| [Ranking Intelligence](/agentic/ranking-intelligence) | User preference embeddings feed into learned ranking model |
...
| Lifecycle Emails | AI-generated insight summaries for personalized email content |
