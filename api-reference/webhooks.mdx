---
title: Webhooks
description: Inbound webhooks from Stripe, Shopify, and Privy, plus 38 async event handlers and 14 cron jobs.
---
...
| `checkout-intent-completed` | [Universal Checkout](/agentic/universal-checkout) intent reached `completed` | Update order records, notify the buyer, write a `PlatformClassificationOutcome` row for feedback into [Merchant Intelligence](/platform/merchant-intelligence) |
| `checkout-intent-failed` | Universal Checkout intent reached `failed` | Log failure code, notify the agent surface, write a `PlatformClassificationOutcome` row |
...
| `enrichment-discover-brands` | A new merchant URL is observed | Fanout work for the [Merchant Intelligence](/platform/merchant-intelligence) brand-discovery pipeline |
| `enrichment-classify-brand-platform` | A `BrandProfile` needs classification | Runs the three-tier platform classifier and updates `ecomPlatform` / `ecomPlatformConfidence` |
| `image-content-validation` | Catalog image enrichment fetched a candidate image | Async revalidation of dimensions and content-type. See [Enrichment Pipeline → Catalog Image Enrichment](/agentic/enrichment-pipeline#catalog-image-enrichment) |
| `scrape-probe-canary` | Scrape pipeline shadow path is sampled | Runs a canary against an alternate extraction tier and records outcome for routing decisions |
...
| `brand-discovery-fanout` | Hourly | Discover new merchant brands from observed traffic | Queues `enrichment-discover-brands` work for newly seen apex domains |
| `brand-classification-drift` | Daily | Re-classify brands whose `reclassifyAfter` has passed | Triggers `enrichment-classify-brand-platform` for drift candidates. See [Merchant Intelligence](/platform/merchant-intelligence#drift-and-re-classification) |
| `campaign-monitor` | Every 15 min | End campaigns past their deadline | Sets `ENDED` status, triggers `campaign-ended` event |
| `companion-nudge` | Periodic | Generate proactive companion re-engagement nudges | Selects eligible users, composes a personalized nudge, and writes it into the conversation history |
...
| `rye-intent-poller` | Periodic | Reconcile in-flight Rye-adapter checkout intents | Polls the Rye adapter for state on `processing` and `confirming` intents, advances the state machine, and emits `checkout-intent-completed` or `checkout-intent-failed`. Only intents whose `adapter` is `rye` are touched today; other adapters reconcile through their own paths. |
| `sage-reengagement` | Periodic | Companion lifecycle re-engagement for inactive users | App-specific cron used by the Sage reference app for re-engagement messaging |
| `sage-sessions-free-refresh` | Daily at UTC midnight | Refresh free-tier session balances for the Sessions paywall | Resets the daily session counter so `sessionsExhausted` clears on schedule |
...
| `POST` | `/webhooks/events/{event}` | Async event handler (38 queues) |
| `POST` | `/webhooks/cron/{job}` | Cron handler (14 jobs) |
