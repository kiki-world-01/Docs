...
Conversion transfers chat history, intent profile data, and agent memory from the guest to the real user. The guest record is deleted after successful merge. Unconverted guests are automatically cleaned up after 7 days.
...
Returns a Stripe checkout URL. On successful payment, the user's tier transitions to `TRIALING` (if trial eligible) or `ACTIVE`.
