...
```typescript SDK
const profile = await podium.companion.createProfile(userId, {
  skinType: "Sensitive",
  concerns: ["Anti-aging", "Hydration"],
  priceRange: { min: 75, max: 150 },
  brands: ["La Mer", "Drunk Elephant"],
...
```typescript SDK
const recs = await podium.companion.getRecommendations(userId, {
  count: 5,
  category: "serum",
});
```
...
<Steps>
  ...
  <Step title="Refine Over Time">
    ...
    ```typescript
    await podium.companion.awardPoints(userId, {
      amount: 10,
