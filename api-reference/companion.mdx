...
```typescript HTTP
const res = await fetch(`https://api.podiumcommerce.xyz/api/v1/companion/profile/${userId}`, {
  method: "POST",
  headers: {
...
```typescript HTTP
await fetch("https://api.podiumcommerce.xyz/api/v1/companion/interactions", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.PODIUM_API_KEY}`,
    "Content-Type": "application/json",
...
const res = await fetch(
  `https://api.podiumcommerce.xyz/api/v1/companion/recommendations/${userId}?count=5&category=serum`,
...
```typescript HTTP
const res = await fetch("https://api.podiumcommerce.xyz/api/v1/companion/orders", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.PODIUM_API_KEY}`,
    "Content-Type": "application/json",
...
<Steps>
  ...
  <Step title="Get Recommendations">
    ...
    ```typescript
    const res = await fetch(
      `https://api.podiumcommerce.xyz/api/v1/companion/recommendations/${userId}?count=5`,
      { headers: { Authorization: `Bearer ${process.env.PODIUM_API_KEY}` } }
    );
    const recs = await res.json();
    ```
  ...
  <Step title="Refine Over Time">
    ...
    ```typescript
    await fetch(`https://api.podiumcommerce.xyz/api/v1/companion/profile/${userId}/points`, {
      method: "POST",
      headers: {
        Authorization: `Bearer ${process.env.PODIUM_API_KEY}`,
        "Content-Type": "application/json",
