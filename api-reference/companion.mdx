...
```bash
curl "https://api.podiumcommerce.xyz/api/v1/companion/agent/chat/history/{userId}?limit=50" \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl https://api.podiumcommerce.xyz/api/v1/companion/creator/{handle}/products \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
```bash
curl -X POST https://api.podiumcommerce.xyz/api/v1/companion/profile/{userId}/seed-from-creator \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "creatorHandle": "janedoe"
