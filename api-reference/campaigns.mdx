...
```bash cURL
curl -X POST https://api.podiumcommerce.xyz/api/v1/campaign \
...
```bash cURL
curl -X PUT https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz \
  -H "Authorization: Bearer $PODIUM_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```typescript SDK
const updated = await podium.campaigns.updateDraft("clcamp_xyz", {
  title: "Which shade is your vibe?",
  description: "Vote on your favorite lip shade and earn 50 points!",
  startDate: "2026-04-01T00:00:00.000Z",
  endDate: "2026-04-30T23:59:59.000Z",
...
```bash
curl -X PATCH https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz/submit \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
```bash
curl -X PATCH https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz/publish \
  -H "Authorization: Bearer $PODIUM_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X PATCH https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz \
  -H "Authorization: Bearer $PODIUM_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash cURL
curl -X POST https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz/vote \
  -H "Authorization: Bearer $PODIUM_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```typescript SDK
await podium.campaigns.vote("clcamp_xyz", {
  userId: "clxyz1234567890",
  votes: [
    { attributeId: 1, optionId: 3 },
    { attributeId: 2, optionId: [1, 4] }
...
```bash cURL
curl -X POST https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_survey/survey \
  -H "Authorization: Bearer $PODIUM_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "clxyz1234567890",
...
```typescript SDK
await podium.campaigns.submitSurvey("clcamp_survey", {
  userId: "clxyz1234567890",
  responses: [
    { questionId: 1, answer: "Hydrating serums" },
    { questionId: 2, answer: ["Sensitive", "Dry"] },
...
```bash
curl https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_survey/survey/report \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
```bash
curl -X PUT https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz/nft \
  -H "Authorization: Bearer $PODIUM_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz/analytics \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
```bash
curl https://api.podiumcommerce.xyz/api/v1/campaign/clcamp_xyz/attributes \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
