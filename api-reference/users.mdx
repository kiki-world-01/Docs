...
```typescript SDK
const updated = await podium.users.update("clxyz1234567890", {
  username: "jane_doe",
  bio: "Clean beauty enthusiast",
  avatarUrl: "https://cdn.example.com/avatar.jpg",
  categoryIds: [1, 3, 7],
...
```typescript SDK
const user = await podium.users.getByPrivyId("did:privy:abc123");
```
...
```typescript SDK
const order = await podium.orders.create("clxyz1234567890", {
  productId: "clprod_abc123",
  variantIds: ["clvar_size_m", "clvar_color_black"],
  quantity: 2
});
