...
```bash cURL
curl "https://api.podiumcommerce.xyz/api/v1/search?query=hyaluronic+serum&type=product&limit=20&page=1" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```

```typescript SDK
const results = await podium.search.query({
...
```bash cURL
curl "https://api.podiumcommerce.xyz/api/v1/universal-search?query=best+moisturizer+for+dry+skin&entityTypes=product,campaign&llmMode=auto&limit=10" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```

```typescript SDK
const results = await podium.search.universal({
...
```bash
curl "https://api.podiumcommerce.xyz/api/v1/universal-search/suggestions?query=hya&limit=5" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
```bash
curl "https://api.podiumcommerce.xyz/api/v1/autocomplete?query=clea&limit=10" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
```bash cURL
curl "https://api.podiumcommerce.xyz/api/v1/discover?suggested=true&page=1&limit=20" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```

```typescript SDK
const feed = await podium.search.discover({
  suggested: true,
  page: 1,
  limit: 20
});
...
```bash cURL
curl "https://api.podiumcommerce.xyz/api/v1/for-you/feed?userId=clxyz1234567890&page=1" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```

```typescript SDK
const feed = await podium.search.forYou("clxyz1234567890", {
  page: 1
});
```
...
```bash
curl "https://api.podiumcommerce.xyz/api/v1/for-you/creators?userId=clxyz1234567890" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
```bash
curl -X POST https://api.podiumcommerce.xyz/api/v1/saved-item \
  -H "Authorization: Bearer $PODIUM_API_KEY" \
...
```bash
curl "https://api.podiumcommerce.xyz/api/v1/saved-item?userId=clxyz1234567890" \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
```bash
curl -X DELETE https://api.podiumcommerce.xyz/api/v1/saved-item/clsaved_abc \
  -H "Authorization: Bearer $PODIUM_API_KEY"
```
...
The universal search layer adds optional LLM synthesis using Anthropic Claude to generate conversational answers grounded in the search results.
...
| `GET` | `/universal-search` | Unified search with optional LLM |
