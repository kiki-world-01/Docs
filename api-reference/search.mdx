...
const res = await fetch(
  "https://api.podiumcommerce.xyz/api/v1/discover?suggested=true&page=1&limit=20",
...
```typescript HTTP
const res = await fetch(
  "https://api.podiumcommerce.xyz/api/v1/for-you/feed?userId=clxyz1234567890&page=1",
  { headers: { Authorization: `Bearer ${process.env.PODIUM_API_KEY}` } }
);
const feed = await res.json();
```
