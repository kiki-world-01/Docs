...
```bash cURL
curl -X POST https://podium-staging.up.railway.app/api/v1/tasks \
...
```bash
curl https://podium-staging.up.railway.app/api/v1/solver/tasks \
  -H "Content-Type: application/json"
```

```bash
curl -X POST https://podium-staging.up.railway.app/api/v1/solver/tasks/{taskId}/claim \
...
```bash
curl -X POST https://podium-staging.up.railway.app/api/v1/solver/tasks/{taskId}/submit \
  -H "Authorization: Bearer $SOLVER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
| **AI Eval** | Claude evaluates submission against criteria | Content tasks, reviews, UGC |
