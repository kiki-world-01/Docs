...
```bash
curl -X POST https://api.podiumcommerce.xyz/api/v1/companion/subscription/checkout \
...
```bash
curl -X POST https://api.podiumcommerce.xyz/api/v1/companion/subscription/portal \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "userId": "clxyz1234567890"
...
```bash
curl https://api.podiumcommerce.xyz/api/v1/companion/subscription/{userId}/status \
  -H "Authorization: Bearer YOUR_API_KEY"
```
...
const status = await fetch(
  `https://api.podiumcommerce.xyz/api/v1/companion/subscription/${userId}/status`,
...
if (status.isSubscribed) {
  ...
  const checkout = await fetch(
    'https://api.podiumcommerce.xyz/api/v1/companion/subscription/checkout',
    {
      method: 'POST',
      headers: {
