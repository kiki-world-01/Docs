...
<Steps>
  <Step title="Sign up">
    Head to the [Podium Developer Portal](https://app.podium.build/onboarding) and sign in with email, Google, or a crypto wallet (powered by Privy).
  </Step>
...
```typescript Fetch
const res = await fetch('https://api.podium.build/api/v1/products', {
  headers: {
...
<CardGroup cols={2}>
  <Card title="Interactive Docs" icon="play" href="https://api.podium.build/docs">
    Try endpoints live with the Scalar API playground — paste your key and send requests
  </Card>
  <Card title="OpenAPI Spec" icon="file-code" href="https://api.podium.build/openapi.json">
...
From the [Dashboard](https://app.podium.build/dashboard/settings/api-keys) you can:
