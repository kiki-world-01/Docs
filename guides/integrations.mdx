...
<Steps>
  ...
  <Step title="Initial sync">
    ...
    This dispatches a `shopify-products-sync` QStash event that fetches all products from the Shopify store.
  </Step>
</Steps>
...
Product data flows from Shopify into Podium. Order data flows from Podium back to Shopify via the `shopify-order-push` QStash event.
...
| Webhook failures | Check the QStash dead letter queue for failed `shopify-products-sync` events |
...
```bash
GET /api/v1/order/{id}/shipping/quote
```

Returns available shipping rates from Shippo based on the order's weight, dimensions, and destination:
...
```bash
POST /api/v1/creator/id/{creatorId}/order/{orderId}/shipping/label
```

Generates and purchases a shipping label via Shippo. Returns the label URL and tracking number.
...
| [QStash](https://upstash.com/docs/qstash) | Async messaging, retries, scheduling |
| [Firecrawl](https://docs.firecrawl.dev/) | Web crawling for enrichment |
