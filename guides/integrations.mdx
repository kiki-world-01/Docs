...
The system keeps email contact records in sync with the latest user data:

| Property | Description | Updated When |
|---|---|---|
| `firstName` | User's first name | Account creation, profile update |
| `skinType` | Primary skin type | Quiz completion, profile update |
| `sageInsightSummary` | AI-generated insight paragraph from [Agent Memory](/agentic/memory-intelligence#email-insight-surface) | After every summary job |
...
Templates use double-brace variables (e.g., `{{firstName}}`, `{{sageInsightSummary}}`) to personalize content. The `sageInsightSummary` variable is particularly powerful — it contains a multi-sentence, first-person narrative generated from the user's structured conversation memory.
...
- **Combine with [Guest Experience](/agentic/guest-experience)** — search first, then seamlessly transition into a guest chat session
