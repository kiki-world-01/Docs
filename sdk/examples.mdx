...
// Tokenize a card with Stripe Elements / Stripe.js using config.publishableKey
// (omitted here). For this snippet, assume you already have a Confirmation Token.
...
```typescript
// 1. Create guest session (no auth needed for the guest)
...
// 3. After the user signs up, merge their guest data
