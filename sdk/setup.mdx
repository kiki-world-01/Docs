...
| `client.companion` | Companion agent: chat, history, recommendations, products, profiles, creator personas, the companion-guest session (and convert), and the World Mini App nonce. The world nonce is a single generated method directly under `client.companion`, not a sub-namespace. |
...
<Note>
Some admin and `/companion/*` operations (e.g. world nonce, agent chat, subscription endpoints, admin API-keys CRUD, admin creator-personas) are reachable through the public REST API but are not yet covered by the generated SDK because they use Elysia `detail` instead of `*_OPERATION`. Use HTTP examples for those routes until the OpenAPI migration completes.
</Note>
