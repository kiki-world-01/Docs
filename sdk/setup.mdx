...
| `client.companion` | Companion agent. Generated methods include `createAgent` (chat), `listAgent` (chat history), `listRecommendations`, `listProducts`, `listProfile` / `createProfile` / `updateProfile`, `listProfileRoutine` / `replaceProfileRoutine`, `createProfileSeedFromCreator`, `listCreator`, `listOrders` / `createOrders` / `updateOrders`, and `companionGuestSession`. |
| `client.companionSubscription` | Stripe-backed subscription billing: checkout, customer portal, status. |
| `client.checkoutIntents` | [Universal Checkout](/agentic/universal-checkout) intents: `createCheckoutIntent`, `getCheckoutIntent`, `confirmCheckoutIntent`, `getCheckoutIntentPaymentConfig`, `listCheckoutIntents`. |
| `client.guests` | Standalone `/guest/order` flow for guest checkout against the commerce catalog. **Not** the same as the companion-guest session. The companion-guest session lives on `client.companion`. |
| `client.guest` | Standalone `/guest/:id` user updates (e.g. setting an email). **Not** the companion-guest session. |
...
<Note>
**`client.companion` vs `client.guests`.** The companion guest session (`POST /companion/guest/session` and `POST /companion/guest/convert`) lives under `client.companion`. It shares a chat user, conversation history, and profile with authenticated users. `client.guests` is a separate, standalone flow for guest **orders** on the commerce API (`/guest/order/*`) and has no relationship to companion chat. If you're building a chat-first onboarding, you want `client.companion`.
</Note>

<Note>
A few routes are reachable through the public REST API but are not yet covered by the generated SDK because they use Elysia `detail` instead of the `*_OPERATION` pattern that drives codegen. Use the HTTP examples in their reference pages until the OpenAPI migration completes:

- `POST /companion/world/nonce` (the World Mini App sign-in nonce)
- `POST /companion/guest/convert` (only the `/session` half is currently generated as `companionGuestSession`)
- `POST /admin/api-keys`, `PATCH/DELETE /admin/api-keys/:id`, `POST /admin/api-keys/:id/rotate`
- `GET /admin/creator-personas/active` and related admin creator-persona endpoints
</Note>
...
Check the [Podium API Playground](https://api.podium.build/docs) for the latest endpoint documentation. The playground is always in sync with the production API.
